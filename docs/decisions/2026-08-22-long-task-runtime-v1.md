...
* **Git-source release path:** the repository does not commit generated `dist/` output. It therefore owns a `prepare: pnpm build` lifecycle script so `dsh plugin --profile web add github:ynymhrb/long-horizon-runtime` can install a loadable package. The README must disclose pnpm's build-approval prompt and retain tarball and local-checkout alternatives.
