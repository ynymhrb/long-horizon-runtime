...
* **Published surface:** the build writes `dist/`, exactly matching the package export map. `dist/` is committed with a release so a Git-hosted DSH plugin loads without invoking a build in the target profile; `pnpm pack --dry-run` and an ESM import are release checks.
...
* **Git-source release path:** follow DSH's prebuilt-plugin pattern: commit the generated `dist/` bundle and ship no `prepare` hook. This avoids package-manager build approval and avoids resolving this repository's private local DSH development links in an end-user profile. The supported Git command pins `#master` until the public repository's default branch is aligned. Every source release must run `pnpm build` and commit the resulting `dist/` changes before Git-source installation is advertised.
