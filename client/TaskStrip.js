...
export function TaskStrip({ sessionId, remote, onOpen }) {
  ...
  React.useEffect(() => { if (!sessionId) return; let live = true; const load = () => Promise.resolve(remote.getCurrentTaskForSession({ sessionId })).then(value => { if (live) setTask(value) }).catch(() => { if (live) setTask(null) }); void load(); const timer = setInterval(load, 3000); return () => { live = false; clearInterval(timer) } }, [sessionId, remote])
  if (!task) return null
