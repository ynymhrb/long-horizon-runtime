...
export function TaskCockpit({ task, graph, events, onBack, remote, sessionId, onTaskChanged }) {
  ...
  const action = label => { setPending(true); setError(null); Promise.resolve(remote.updateTask({ taskId: task.id, expectedRevision: task.controlRevision, action: label, ...(sessionId ? { sessionId } : {}) })).then(result => { const value = remoteValue(result); onTaskChanged(value.kind === 'conflict' ? value.current : value.task) }).catch(reason => setError(String(reason))).finally(() => setPending(false)) }
