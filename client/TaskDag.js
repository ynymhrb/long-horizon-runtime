...
import { layoutTaskGraph, NODE_HEIGHT, NODE_WIDTH, stableCanvasSize, taskNodeLabelPresentation, truncateGraphText, visibleTaskGraph } from './task-graph.js'
  ...
  return e('section', { className: 'ltr-dag-wrap' },
    ...
    e('svg', { className: 'ltr-dag', viewBox: `0 0 ${canvas.width} ${canvas.height}`, preserveAspectRatio: 'xMinYMin meet', onWheel: event => { event.preventDefault(); setView(value => ({ ...value, scale: Math.min(1.8, Math.max(.55, value.scale + (event.deltaY < 0 ? .1 : -.1))) })) }, onPointerDown: event => { if (event.target === event.currentTarget) drag.current = { x: event.clientX, y: event.clientY } }, onPointerMove: event => { if (drag.current) { const dx = event.clientX - drag.current.x; const dy = event.clientY - drag.current.y; drag.current = { x: event.clientX, y: event.clientY }; setView(value => ({ ...value, x: value.x + dx, y: value.y + dy })) } }, onPointerUp: () => { drag.current = null } },
      e('g', { transform: `translate(${view.x} ${view.y}) scale(${view.scale})` },
        ...
        ...graph.nodes.map(node => { const state = taskStatePresentation(node.state); const title = taskNodeLabelPresentation(node); const selected = node.id === selectedId; const collapsed = collapsedIds.has(node.id); const hiddenCount = visible.hiddenBy.get(node.id)?.length ?? 0; return e('g', { key: node.id, className: `ltr-node tone-${state.tone}${selected ? ' is-selected' : ''}`, transform: `translate(${node.x} ${node.y})`, role: 'button', tabIndex: 0, 'aria-label': `${node.id} ${title.title}`, onClick: () => onSelect(node.id), onKeyDown: event => { if (event.key === 'Enter' || event.key === ' ') onSelect(node.id) } }, e('rect', { width: NODE_WIDTH, height: NODE_HEIGHT, rx: 10 }), e('text', { x: 12, y: 25, className: 'ltr-node-title' }, truncateGraphText(node.id, 19)), e('text', { x: 12, y: 47, className: 'ltr-node-objective' }, e('title', null, title.title), truncateGraphText(title.label, 14)), e('text', { x: 12, y: 64, className: 'ltr-node-state' }, truncateGraphText(state.label, 14)), hasChildren(node.id) ? e('g', { className: 'ltr-collapse-control', role: 'button', tabIndex: 0, 'aria-label': `${collapsed ? '展开' : '折叠'} ${node.id} 下游任务`, onClick: event => { event.stopPropagation(); toggle(node.id) }, onKeyDown: event => { if (event.key === 'Enter' || event.key === ' ') { event.stopPropagation(); toggle(node.id) } } }, e('rect', { x: NODE_WIDTH - 31, y: 9, width: 21, height: 18, rx: 5 }), e('text', { x: NODE_WIDTH - 20, y: 23, textAnchor: 'middle' }, collapsed ? `+${hiddenCount}` : '−')) : null) }),
      ),
    ),
  )
}
