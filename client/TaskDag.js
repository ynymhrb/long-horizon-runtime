...
import { layoutTaskGraph, NODE_HEIGHT, NODE_WIDTH, visibleTaskGraph } from './task-graph.js'
...
export function TaskDag({ nodes, selectedId, onSelect }) {
  ...
  return e('section', { className: 'ltr-dag-wrap' },
    e('div', { className: 'ltr-dag-tools' }, e('div', null, e('button', { type: 'button', onClick: fit }, '适应视图'), e('button', { type: 'button', onClick: () => setCollapsedIds(new Set()) }, '全部展开'), e('button', { type: 'button', onClick: () => setCollapsedIds(new Set(nodes.filter(node => hasChildren(node.id)).map(node => node.id))) }, '全部折叠')), e('span', null, `${graph.nodes.length}/${nodes.length} 个节点`)),
