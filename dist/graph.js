...
export function validatePlan(draft) {
    if (typeof draft.goalId !== 'string' || draft.goalId.trim().length === 0)
        throw new PlanValidationError('goal id must not be empty');
    if (!Number.isSafeInteger(draft.revision) || draft.revision < 1)
        throw new PlanValidationError('revision must be a positive integer');
    if (draft.tasks.length === 0)
        throw new PlanValidationError('plan must contain at least one task');
    const tasks = new Map();
    for (const task of draft.tasks) {
        if (!task || typeof task !== 'object')
            throw new PlanValidationError('task must be an object');
        for (const field of ['priority', 'inputContract', 'outputContract', 'completionCriteria', 'retryPolicy', 'sideEffectClass', 'validator'])
            if (task[field] === undefined)
                throw new PlanValidationError(`task ${String(task.id)} ${field} is required`);
        if (!Array.isArray(task.dependsOn) || task.dependsOn.some(id => typeof id !== 'string' || id.trim().length === 0))
            throw new PlanValidationError(`task ${String(task.id)} has invalid dependencies`);
        if (task.id.trim().length === 0)
            throw new PlanValidationError('task id must not be empty');
        ...
        if (task.priority !== undefined && (!Number.isSafeInteger(task.priority)))
            throw new PlanValidationError(`task ${task.id} priority must be a safe integer`);
        if (task.sideEffectClass !== undefined && !['read_only', 'idempotent', 'external_effect'].includes(task.sideEffectClass))
            throw new PlanValidationError(`task ${task.id} has invalid sideEffectClass`);
        if (task.retryPolicy !== undefined && (!Number.isSafeInteger(task.retryPolicy.maxAttempts) || task.retryPolicy.maxAttempts < 1))
            throw new PlanValidationError(`task ${task.id} has invalid retry policy`);
        if (task.timeoutMs !== undefined && (!Number.isSafeInteger(task.timeoutMs) || task.timeoutMs < 1))
            throw new PlanValidationError(`task ${task.id} has invalid timeoutMs`);
        for (const [name, contract] of [['inputContract', task.inputContract], ['outputContract', task.outputContract]])
            if (contract !== undefined && (contract === null || Array.isArray(contract) || typeof contract !== 'object'))
                throw new PlanValidationError(`task ${task.id} ${name} must be an object`);
        if (task.completionCriteria !== undefined && task.completionCriteria.trim().length === 0)
            throw new PlanValidationError(`task ${task.id} completionCriteria must not be empty`);
        if (task.validator !== undefined && task.validator.trim().length === 0)
            throw new PlanValidationError(`task ${task.id} validator must not be empty`);
        tasks.set(task.id, {
            ...task,
    ...
    for (const task of tasks.values()) {
        for (const dependency of task.dependsOn) {
            if (!tasks.has(dependency))
                throw new PlanValidationError(`task ${task.id} depends on missing task ${dependency}`);
        }
    }
    assertAcyclic(tasks);
...
export function applyMutation(current, mutation) {
    const next = [...current.tasks.values()].map(task => ({ ...task, dependsOn: [...task.dependsOn] }));
    switch (mutation.kind) {
        case 'invalidateTask': {
            if (!current.tasks.has(mutation.taskId))
                throw new PlanValidationError(`unknown task ${mutation.taskId}`);
        ...
        case 'replaceTask': {
            ...
            if (index < 0)
                throw new PlanValidationError(`unknown task ${mutation.taskId}`);
            if (mutation.replacement.id !== mutation.taskId)
                throw new PlanValidationError('replacement task id must equal the replaced task id');
    ...
    for (const [id, previous] of current.tasks) {
        if (mutation.kind !== 'replaceTask' || id !== mutation.taskId) {
            ...
            if (task !== undefined)
                withInvalidation.set(id, { ...task, state: previous.state });
        }
    }
    if (mutation.kind === 'invalidateTask') {
        for (const id of reachableFrom(current.tasks, mutation.taskId)) {
            const task = withInvalidation.get(id);
            if (task !== undefined)
                withInvalidation.set(id, { ...task, state: 'INVALIDATED' });
        }
    }
    ...
    if (mutation.kind === 'replaceTask' || mutation.kind === 'addEdge') {
        ...
        for (const id of reachableFrom(current.tasks, root)) {
            const task = withInvalidation.get(id);
            if (task !== undefined)
                withInvalidation.set(id, { ...task, state: 'PENDING' });
        }
    }
...
function assertAcyclic(tasks) {
    ...
    const visit = (id) => {
        if (visiting.has(id))
            throw new PlanValidationError(`dependency cycle includes task ${id}`);
        if (visited.has(id))
            return;
        visiting.add(id);
...
function reachableFrom(tasks, root) {
