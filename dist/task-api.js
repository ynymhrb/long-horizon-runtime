...
function summarizeEvent(event) {
    ...
    for (const [key, value] of Object.entries(event.payload))
        if (!EVENT_PAYLOAD_EXCLUDED_KEYS.has(key))
            payload[key] = value;
...
export class TaskControlApi {
    ...
    async create(request, invocation) {
    ...
    async attachSession(taskId, invocation) {
    ...
    setCurrentSessionTask(taskId, invocation) {
        const current = this.requireTask(taskId);
        this.assertScope(current.workspaceScope, invocation.workspaceScope);
        if (!current.sessionLinks.some(link => link.sessionId === invocation.sessionId))
            throw new Error(`session ${invocation.sessionId} is not attached to task ${taskId}`);
    ...
    clearCurrentSessionTask(sessionId) {
    ...
    async update(request, invocation) {
        const current = this.requireTask(request.taskId);
        this.assertScope(current.workspaceScope, invocation.workspaceScope);
        if (request.expectedRevision !== current.controlRevision)
            return { kind: 'conflict', current };
        let task;
        switch (request.action) {
            case 'confirm':
                task = await this.runtime.confirmGoal(request.taskId, invocation.parent, invocation.signal);
                break;
            case 'resume':
                task = await this.runtime.resumeGoal(request.taskId, invocation.parent, request.recoveryResolution, invocation.signal);
                break;
            case 'cancel':
                task = this.runtime.cancelGoal(request.taskId);
                break;
            case 'pause':
                task = this.pause(request.taskId);
                break;
        }
    ...
    async editGoal(request, invocation) {
        const current = this.requireTask(request.taskId);
        this.assertScope(current.workspaceScope, invocation.workspaceScope);
        if (request.expectedRevision !== current.controlRevision)
            return { kind: 'conflict', current };
    ...
    async acceptReplan(request, invocation) {
        const current = this.requireTask(request.taskId);
        this.assertScope(current.workspaceScope, invocation.workspaceScope);
        if (request.expectedRevision !== current.controlRevision)
            return { kind: 'conflict', current };
    ...
    get(taskId, invocation) {
        const task = this.runtime.getStatus(taskId);
        if (task !== undefined && invocation?.workspaceScope !== undefined)
    ...
    listAttemptSessions(request, invocation) {
        const task = this.runtime.getStatus(request.taskId);
        if (task === undefined)
            return null;
        this.assertScope(task.workspaceScope, invocation.workspaceScope);
    ...
    interrupt(taskId, cause, recoveryOutcome) {
        return this.runtime.interruptGoal(taskId, cause, recoveryOutcome);
    }
    proposeReplan(taskId, mutation) { return this.runtime.proposeReplan(taskId, mutation); }
    rejectReplan(taskId) { return this.runtime.rejectReplan(taskId); }
    /** Reject only the proposal observed at this control revision; never discard a newer plan. */
    rejectReplanAtRevision(taskId, expectedRevision) {
        const current = this.requireTask(taskId);
        if (current.controlRevision !== expectedRevision)
            return { kind: 'conflict', current };
     ...
     */
    continueInSession(taskId, invocation) {
        const current = this.requireTask(taskId);
        this.assertScope(current.workspaceScope, invocation.workspaceScope);
        if (!current.sessionLinks.some(link => link.sessionId === invocation.sessionId))
            this.runtime.attachSession(taskId, invocation.sessionId);
        if (this.runtime.store.getCurrentTaskForSession(invocation.sessionId)?.taskId !== taskId)
            return this.setCurrentSessionTask(taskId, invocation).task;
        return this.requireTask(taskId);
    }
    ...
    bindOnContinue(taskId, action, invocation) {
        const sessionId = invocation.sessionId;
        if (sessionId === undefined || (action !== 'confirm' && action !== 'resume'))
            return undefined;
    ...
    advance(taskId, currentRevision) {
    ...
    requireTask(taskId) { const task = this.runtime.getStatus(taskId); if (task === undefined)
        throw new Error(`unknown task ${taskId}`); return task; }
    assertScope(taskScope, invocationScope) {
        if (taskScope !== undefined && invocationScope !== undefined && taskScope !== invocationScope)
            throw new Error('task belongs to a different workspace scope');
    }
}
